...
aLT = CALCtools.produceGradient(torch.from_numpy(x), torch.from_numpy(T)).cpu().numpy()
axs[1, 0].plot(x, aLT, "-", c="b")

print(f"<T> Parabolic = {FUNCTIONALScalc.calculate_simplified_volavg(x,T)[0]:.3f}keV")

# axs[0,1].plot(x,n,'-',c='b')
# aLn = CALCtools.produceGradient(torch.from_numpy(x),torch.from_numpy(n)).cpu().numpy()
