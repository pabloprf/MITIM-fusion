...
# --------------------------------------------------------------------------------------------------------
#  Ready to go optimization tool: PRF sequential method
# --------------------------------------------------------------------------------------------------------
...
def prfseq(
    ...
    print(
        f"* Flux-grad relationship of {relax} and maximum gradient jump of {dx_max*100.0:.1f}%, to achieve channel residual of {tol:.1e} in {max_it:.0f} iterations"
    )

    x = copy.deepcopy(xGuess)
    Q, QT = flux(x, cont=0)
    print(
        f"* Starting overall residual: {(Q-QT).abs().mean(axis=1)[0].item():.4e}, will run {int(max_it)-1} more evaluations",
        typeMsg="i",
    )
    ...
    cont = 0
    for i1, prof in enumerate(info_case["profs"]):
        for i2, rho in enumerate(info_case["rho"]):
            print(
                f"\t- Minimizing residual of {prof} at rho = {rho}, Q = {Q[0,cont].item():.2f}, QT = {QT[0,cont].item():.2f}",
                typeMsg="i",
            )
            ...
            for i in range(int(max_it) - 1):
                ...
                resChannel = (Q[:, cont] - QT[:, cont]).abs()[0].item()

                if (i + 1) % int(print_each) == 0:
                    print(
                        f"\t\t- it#{i+1}: Gradient jump of {(x_new[0,cont]-x_orig[0,cont])/x_orig[0,cont]*100.0:.2f}%, residual of {resChannel:.1e}/{tol:.1e} (Q = {Q[0,cont].item():.2f}, QT = {QT[0,cont].item():.2f})"
                    )
            ...
            print(
                f"\t\t* Gradient varied by {(x_new[0,cont]-x_orig[0,cont])/x_orig[0,cont]*100.0:.2f}, achieved channel residual of {resChannel:.1e}, overall residual of {(Q-QT).abs().mean(axis=1)[0].item():.1e}"
            )

            cont += 1

    print(
        f"* Final overall residual: {(Q-QT).abs().mean(axis=1)[0].item():.1e}",
        typeMsg="i",
    )
