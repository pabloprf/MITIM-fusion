...
class CGYRO:
    ...
    def run_full(
        ...
        n = 16,
        nomp = 1,
        mem = None, # in MB, e.g. 16000 for 16GB
        queue = None, # e.g. "-queue regular" for NERSC (only if submitting via job array)
        ...
        if submit_via_qsub:
            ...
            CGYROcommand = f'gacode_qsub -e {subfolder} -n {n} -nomp {nomp} {queue} -mem {mem} -w 0:{minutes}:00 -s'
            ...
            if queue is not None:
                self.cgyro_job.slurm_settings['partition'] = queue
