"""
This example reads NEO from an already existing folder (no normalizations if no input_gacode file provided)

	read_neo.py run0/ [--suffixes _0.55] [--gacode input.gacode]
...
def main():
    ...
    neo = NEOtools.NEO()
    neo.prep_from_file(
        folders[0], folders[0] / f"input.neo{suffixes[0]}", input_gacode=input_gacode
    )
    for i, folder in enumerate(folders):
        neo.read(folder=folder, suffix=suffixes[i], label=f"run{i}")

    fn = FigureNotebook("NEO MITIM Notebook", geometry="1700x900", vertical=True, show=not noshow)
    neo.plot(fn=fn, labels=[f"run{i}" for i in range(len(folders))])
