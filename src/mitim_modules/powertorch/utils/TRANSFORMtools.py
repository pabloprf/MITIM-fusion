...
def gacode_to_powerstate(self, rho_vec=None):
    ...
    if self.target_options["ModelOptions"]["TypeTarget"] < 3:
    ...
    if self.target_options["ModelOptions"]["TypeTarget"] < 2:
        # Exchange fixed if 1
...
def powerstate_to_gacode_powers(self, profiles, position_in_powerstate_batch=0):
    ...
    with LOGtools.HiddenPrints():
        state_temp.__init__(
            ...
            target_options={
                ...
                "ModelOptions": {
                    "TypeTarget": self.target_options["ModelOptions"]["TypeTarget"], # Important to keep the same as in the original
    ...
    state_temp.target_options["ModelOptions"]["targets_evaluator_method"] = "powerstate"
    state_temp.calculateTargets()
    # ------------------------------------------------------------------------------------------

    conversions = {}

    if self.target_options["ModelOptions"]["TypeTarget"] > 1:
