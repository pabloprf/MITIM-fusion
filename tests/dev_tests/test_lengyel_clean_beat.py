...
bc(sharpness) beat -- the physics-based separatrix temperature then sets the
scale of the sharpness boundary condition (Te_bc = Tsep/(1 - xi*C)):

    init (FreeGS + fixed BC) -> lengyel (clean) -> bc method=sharpness (xi=1)
...
nml["maestro"]["beats"] = ["lengyel", "bc"]

nml["maestro"]["lengyel"]["parameters_prepare"]["mode"] = "clean"

# Method switch on the template's bc block: its confinement_parameters sub-dict may
# stay in place (ignored under method 'sharpness')
nml["maestro"]["bc"]["parameters_prepare"]["method"] = "sharpness"
nml["maestro"]["bc"]["parameters_prepare"]["sharpness_parameters"]["sharpness"] = 1.0
