...
Key teaching points:
    ...
    3. fetch() retrieves the output files from the run machine and organizes
       ...
       as in a blocking run (see cgyro_01_linear_run_from_inputgacode.py).
    4. Note that CGYRO has a FOURTH settings level on top of the usual
       ...
       (see cgyro_02_nonlinear_run_from_inputgacode.py).
"""
...
# This full process should take in the order of ~40 seconds using 8 cores (example in a MacBook Pro M3 Pro Max)