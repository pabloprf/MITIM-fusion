...
*   🌀 **TRANSP rotation defaults**: TRANSP runs now ship a rotation U-File and enable the NCLASS Er by default (`rotation_source: echo`). Downstream **results are unchanged** for existing zero-rotation chains (`echo` is a pass-through: zero in → zero out), but generated TRANSP namelists gain the rotation flags (inert at zero rotation). `rotation_source: off` reproduces the pre-rotation namelists bit-for-bit; rotation physics in the carried `w0` is strictly opt-in.
