...
*   🔬 **DIII-D experimental data (MDSplus)**: new `mitim_tools/experiment_tools/diiid/` (`retrieval.py` + `plotting.py`) pulls DIII-D time traces, EFIT equilibria and Thomson/CER profiles via the pure-python `mdsthin` thin client (optional extra: `pip install mitim-fusion[mds]`), over ONE reused SSH tunnel with on-disk caching (server-side resampling, polite to atlas). Signals are resolved with the standard DIII-D MDSplus cascade (findsig → ptdata2 → pseudo). `plotting.overview(shots, layout, …)` draws a declarative multi-column layout — scalar time traces, flux-surface equilibria (R,Z), and Te/ne/Ti profiles vs ρ (time-averaged over a chosen analysis window and mapped through each shot's EFIT, with error bars). See `tests/capability_tests/diiid_01_fetch_and_plot.py`.
