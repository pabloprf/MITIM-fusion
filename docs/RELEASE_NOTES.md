...
*   🪚 **MAESTRO TRANSP adaptive sawtooth-period floor** (`min_sawtooth_period_ms`): on compact, cold-core plasmas the Park-Monticello period `tau_PM ~ R^2 * Te0^1.5 / Zeff` — which floors the Porcelli crash interval — is sub-ms, forcing sub-ms timesteps and a multi-GB output CDF. The beat now sets `c_sawtooth(2) = min_sawtooth_period_ms / tau_PM` per case (new `PLASMAtools.park_monticello_sawtooth_period`), leaving large machines untouched. Template default **10 ms**; `null` bypasses.

*   🧩 **MAESTRO TRANSP boundary-surface backoff** (`separatrix.boundary_surface_psin`): the fixed TRANSP boundary can now be taken just inside the separatrix (e.g. `0.995`), which is rounder and clears TRANSP's curvature-ratio abort while preserving the true plasma shape (unlike lowering `n_mxh`). Default `1.0` = separatrix. **NOTE**: this knob and `n_mxh` are now consumed by the FIRST TRANSP beat only, which freezes the boundary curve for all later beats — re-deriving it per beat compounded and silently shrank the plasma ≈1.3% in minor radius over 4 beats. Best used with `separatrix.internal_flux_file`, whose radial shaping decay makes interior surfaces physically rounder; that path now also carries its poloidal-flux mapping through the initializer's FREEGS correction step.
...
*   🐛 **MAESTRO TRANSP Porcelli sawtooth triggering**: runs could finish without ever triggering sawteeth when the Park-Monticello predicted period was long (only 10% of it was taken as the minimum). A user-specified floor is now available (see above).
