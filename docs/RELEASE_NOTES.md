...
*   🌀 **TRANSP toroidal rotation I/O**: (1) the NCLASS neoclassical potential is now written by default (`computeNCLASSpotential=True` -> `nlvwnc=T`), so every TRANSP run carries the neoclassical Er and rotation (`OMEGA_NC`/`EPOTNC`) in the CDF (previously off, so CDFtools returned zeros). (2) `gacode_state.to_transp` now passes the plasma rotation INTO TRANSP as the `omg` U-File — angular frequency in rad/s, a flux function identical to `input.gacode` `w0(rad/s)` (no `Vtor=w0*R` conversion); auto-enabled when `w0` is non-zero (no-op otherwise), which also turns on `UFrotation` (`nlvphi=T`, bulk rotation). In MAESTRO the transp beat's `write_rotation` knob controls both directions — `w0` into TRANSP and TRANSP's rotation back out to the next beat (`null`=auto, `false`=keep the chain at zero rotation / no E×B shear). Previously `w0` was silently dropped on the way into TRANSP. New dev tests: `test_transp_vs_vgen_rotation.py` (TRANSP/NCLASS vs VGEN/NEO neoclassical rotation) and `test_maestro_rotation.py` (rotation flow through a TRANSP→PORTALS→TRANSP→PORTALS chain with `w0` predicted).
